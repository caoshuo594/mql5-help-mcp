...
class ResponseFormatter {
    // 快速模式 (~500 tokens)
    static formatQuick(extracted, analysis, docName) {
        let answer = "";
        // 根据查询类型定制答案
    ...
    // 详细模式 (~1500 tokens)
    static formatDetailed(extracted, analysis, docName, relatedDocs) {
        return {
            type: "detailed",
...
export class SmartQueryEngine {
    docIndex;
    constructor(docIndex) {
        this.docIndex = docIndex;
    }
    // 内部搜索 (不返回给AI)
    internalSearch(keywords, limit = 3) {
        const results = [];
    ...
    // 从错误数据库搜索错误解决方案
    searchErrorDatabase(query, errorCode) {
